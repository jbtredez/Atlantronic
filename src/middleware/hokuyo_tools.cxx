...
int rplidar_find_objects(struct rplidar_scan* scan, Vect2* rplidar_pos, unsigned int size, struct polyline* obj, unsigned int obj_size)
{
	...
	{
		// on passe les points erronés ou en dehors de la table
		while( ( i < size && scan->distance[i] < scan->min_distance) || fabsf(rplidar_pos[i].x) > 1500 - HOKUYO_TABLE_DELTA || fabsf(rplidar_pos[i].y) > 1000 - HOKUYO_TABLE_DELTA)
		{
			i++;
		}
		...
		{
			dist = scan->distance[i];
			if( dist < scan->min_distance || fabsf(rplidar_pos[i].x) > 1500 - HOKUYO_TABLE_DELTA || fabsf(rplidar_pos[i].y) > 1000 - HOKUYO_TABLE_DELTA )
			{
				gap = GAP;
			}
			else
		...
		if(object_size >= scan->min_object_size)
		{
			...
			obj_size--;
			obj->pt = rplidar_pos + object_start;
			obj->size = object_size;
			obj++;
			res++;
		}
