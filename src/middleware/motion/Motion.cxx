...
void Motion::updateUsbData(struct control_usb_data* data)
{
	...
	for(int i = 0; i < MOTION_MOTOR_MAX; i++)
	{
		...
		data->cons_motors_v[i] *= m_motionMotor[i]->inputGain; // TESTS
		data->mes_motors[i] = ((CanMipMotor*)m_motionMotor[i])->kinematics;//m_kinematicsMes[i];
		data->mes_motors[i].v *= m_motionMotor[i]->inputGain; // TESTS : affichage en rpm
