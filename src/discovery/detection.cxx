...
static void detection_task(void* arg)
{
	...
	while(1)
	{
		...
		//		log_format(LOG_INFO, "compute_time : %lu us", dt.ms * 1000 + dt.ns/1000);
			...
			{
				if (detection_object_polyline[i].size)
				{
					detect_size++;
				}
			}
...
static void detection_compute()
{
	...
	detection_num_obj = hokuyo_find_objects(hokuyo[HOKUYO1].scan.distance, detection_hokuyo_pos, HOKUYO_NUM_POINTS, detection_object_polyline, DETECTION_NUM_OBJECT);
	detection_reg_size = 0;
...
static void detection_remove_static_elements_from_dynamic_list()
{
	...
	for(i=0; i<nb_objects_to_test; i++)
	{
		...
		if(current_dyn_object->size == 1)
		{
			...
			if(current_dyn_object == (detection_object_polyline+detection_num_obj-1))
			{
				detection_num_obj--;
			}
		}
